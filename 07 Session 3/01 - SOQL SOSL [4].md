...
Single Words vs. Phrases in SOSL:
    - Single Words:
        - Words separated by spaces (e.g. FIND {Hope}) match any order in the text and do not need to be consecutive.
    - Exact Phrases:
        - Use double quotes for an exact phrase (eg. FIND {"The Hope"}), where words must appear together in the specified order.

Filtering Results:
- Use WHERE clause to narrow down results within each specified object.
- Example: RETURNING Account(Name, Industry WHERE Industry = 'Apparel')

Ordering Results:
- Use ORDER BY to sort results for an object.
- Example: RETURNING Account(Name, Industry ORDER BY Name)

Limiting Results:
- Use LIMIT to restrict the number of results rturned for an object.
- Example: RETURNING Account(Name, Industry LIMIT 10)

Using SOSL in Apex
List<list<SObject>> lstResults = [FIND 'Gen' IN NAME FIELDS RETURNING Account(Id, Name), Contact(Id, FirstName, LastName)];
...
- Use FIND with single quotes in Apex: FIND 'searchTerm'
- SOSL query results in Apex are stored as a List of Lists (List<List<SObject>>).

Accesing Results by Object Type:
- Each list in the result corresponds to an object type specified in RETURNING.
- Example:
    - searchResults[0] contains Account records
...
SOQL
====
- Optimize Filters: Use indexed fields in the WHERE clause to improve performance.
- Limit and Offset: Always use LIMIT to control the number of the returned records.
- Avoid Nested Queries: Limit the use of deeply nested relationship queries to avoid hitting limits.
- Use SELECT Specific Fields.

SOSL
====
- Use Exact Words: SOSL works best when using complete words. Avoid partial words unless using wildscards.
- Limit Returned Objects: Specify only necessary objects in the RETURNING clause to keep results focused.
- Combine Logical Operators: Use AND, OR and wildscards for more flexible search terms.
- Optimize for User Search: Consider indexing fields often searched by users to enhance search speed.